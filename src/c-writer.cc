...
void CWriter::WriteLocals(const std::vector<std::string>& index_to_name) {
  ...
  for (Type type : {Type::I32, Type::I64, Type::F32, Type::F64, Type::V128, Type::FuncRef,
                    Type::ExternRef}) {
    Index local_index = 0;
    size_t count = 0;
...
void CWriter::Write(const SimdLoadLaneExpr& expr) {
  ...
  Memory* memory = module_->memories[module_->GetMemoryIndex(expr.memidx)];
  Type result_type = expr.opcode.GetResultType();
  Write(StackVar(0, result_type), " = ", func, expr.val, "(", ExternalInstancePtr(memory->name), 
        ", (u64)(", StackVar(1), ")");
...
void CWriter::Write(const SimdStoreLaneExpr& expr) {
  ...
  Write(func, expr.val, "(", ExternalInstancePtr(memory->name),
        ", (u64)(", StackVar(1), ")");
  ...
  switch (expr.opcode) {
    case Opcode::I8X16Shuffle: {
      Write(StackVar(1, result_type), " = simde_wasm_i8x16_shuffle(",
        ...
        expr.val.u8(0), ", ",  expr.val.u8(1), ", ",  expr.val.u8(2), ", ",  expr.val.u8(3), ", ", 
        expr.val.u8(4), ", ",  expr.val.u8(5), ", ",  expr.val.u8(6), ", ",  expr.val.u8(7),  ", ", 
        expr.val.u8(8), ", ",  expr.val.u8(9), ", ",  expr.val.u8(10), ", ", expr.val.u8(11), ", ", 
        expr.val.u8(12), ", ", expr.val.u8(13), ", ", expr.val.u8(14), ", ", expr.val.u8(15), ");", 
        Newline());
      DropTypes(2);
      break;
    }
...
void CWriter::Write(const LoadSplatExpr& expr) {
  ...
  Type result_type = expr.opcode.GetResultType();
  Write(StackVar(0, result_type), " = ", func, "(", ExternalInstancePtr(memory->name),
        ", (u64)(", StackVar(0), ")");
  if (expr.offset != 0)
    Write(" + ", expr.offset);
  Write(");", Newline());
